<template>
  <div class="step">
    <div class="step-head">
      <h2>描述您的采购需求</h2>
      <p>用自然语言输入，AI 会自动识别关键信息</p>
    </div>
    <textarea class="apple-textarea" v-model="store.inputText" placeholder="例如：学校拟采购一批人工智能实验室服务器，预算80万元，要求4台GPU服务器，每台不少于2张48GB显存GPU，交付周期30天..." maxlength="5000"></textarea>
    ...
    <div class="step-actions">
      ...
      <button class="apple-btn apple-btn-primary" :disabled="!store.inputText.trim()" @click="store.doParse()" style="padding:12px 28px;font-size:15px;">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
        {{ store.loading ? '解析中...' : '智能解析' }}
      </button>
    </div>
    <div class="samples">
...
.sample-chip { padding: 8px 16px; border-radius: 100px; border: 0.5px solid var(--border); background: rgba(0,0,0,0.02); font-size: 13px; color: var(--text-secondary); cursor: pointer; transition: all var(--transition); font-family: var(--font); }
.sample-chip:hover { border-color: var(--accent); background: var(--accent-light); color: var(--accent); }
</style>
